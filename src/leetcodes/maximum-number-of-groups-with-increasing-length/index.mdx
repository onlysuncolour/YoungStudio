...
[leetcode-长度递增组的最大数目](https://leetcode.cn/problems/maximum-number-of-groups-with-increasing-length/description/)
...
var maxIncreasingGroups = function(usageLimits) {
  let temp = 0, count = 0, len = usageLimits.length
  const _usageLimits = usageLimits.sort((a, b) => a-b);
  for (let index = 0; index < len; index++) {
    if (_usageLimits[index] + temp >= count + 1) {
      count ++;
      temp += _usageLimits[index] - count;
    } else {
      temp += _usageLimits[index]
    }
  }
  return count
}
